import numpy as np
from src.neural_components.activation import Activation
from src.neural_components.cost import Cost
from src.neural_components.optimizers import Optimizers
from src.neural_components.regularization import Regularization
from src.neural_components.normalization import BatchNormalization
from src.neural_components.learning_rate import LearningRateScheduler
...
class NeuralNetwork:
    ...
    def load_config(self, dataset, config, base_config):
        if config is not None and base_config is not None:
            ...
            with open('NN-From-Scratch/src/config/nn_parameters.json', 'r') as f:
                self.config = json.load(f)[dataset]
            with open('NN-From-Scratch/src/config/base_parameters.json', 'r') as f:
